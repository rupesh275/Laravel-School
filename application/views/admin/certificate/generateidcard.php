        ...
        <div class="row">
            <div class="col-md-12">
                <div class="box box-primary">
                    ...
                    if (isset($resultlist)) {
                        ?>
                        <form method="post" action="<?php echo base_url('admin/generateidcard/generatemultiple_pdf') ?>">
                            <div  class="" id="duefee">
                                ...
                                <div class="box-header ptbnull">
                                    ...
                                    <button class="btn btn-info btn-sm pull-right" style="margin-left: 8px;" type="submit" name="generate">Generate Pdf</button>
                                    <button class="btn btn-info btn-sm printSelected pull-right" style="margin-left: 8px;" type="button" name="generate" title="<?php echo $this->lang->line('generate') . " " . $this->lang->line('certificate'); ?>"><?php echo $this->lang->line('generate'); ?></button>
                                    <button class="btn btn-info btn-sm pull-right viewSelected" style="margin-left: 8px;" type="button" name="generate">View Id Card</button>
                                    <button class="btn btn-info btn-sm pull-right countSelected" type="button" name="generate">Update Id Print Count</button>
                                </div>
                                                ...
                                                if (empty($resultlist)) {
                                                    ...
                                                    foreach ($resultlist as $student) {
                                                        ...
                                                        <tr>
                                                            ...
                                                            <?php if ($student['image'] == "" ||$student['image'] == "uploads/student_images/default_male.jpg" || $student['image'] == "uploads/student_images/default_female.jpg") { ?>
                                                                <td><?php echo "No"; ?></td>
                                                                <?php }else{
                                                                    ?>
                                                                     <td><?php echo "Yes"; ?></td>
                                                                ...
                                                                <td><?php echo $student['id_prn_cnt']; ?></td>
                                                            <td>
                                                            ...
                                                            <td><?php echo !empty($student['current_address']) ? $student['current_address'] : $student['permanent_address']; ?></td>
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.viewSelected', function () {
            ...
            if (array_to_print.length == 0) {
                ...
                $.ajax({
                    url: '<?php echo site_url("admin/generateidcard/generatemultipleview") ?>',
                    type: 'post',
                    dataType: 'JSON',
                    data: {'data': JSON.stringify(array_to_print), 'class_id': classId, 'id_card': idCard, },
                    success: function (response) {
                        // $("body").append(response.page);
                        $('html').html(response.page);
                    }
                });
            }
        });

        $(document).on('click', '.countSelected', function () {
            var array_to_print = [];
            var classId = $("#class_id").val();
            var idCard = $("#id_card_id").val();
            $.each($("input[name='check[]']:checked"), function () {
            ...
            if (array_to_print.length == 0) {
                ...
                $.ajax({
                    url: '<?php echo site_url("admin/generateidcard/generatemultipleCount") ?>',
                    type: 'post',
                    dataType: 'JSON',
                    data: {'data': JSON.stringify(array_to_print), 'class_id': classId, 'id_card': idCard,'students_count':array_to_print.length, },
                    success: function (response) {
                        // $("body").append(response.page);
                        // $('html').html(response.page);
