        ...
        <div class="row">
            <div class="col-md-12">
                <div class="box removeboxmius">
                    ...
                    <form role="form" action="<?php echo site_url('admin/feemaster/reconsilation') ?>" method="post" class="">
                        <div class="box-body row" style="display:show">
                            ...
                            <div class="col-sm-3 col-lg-3 col-md-3">
                                <div class="form-group">
                                    <label><?php echo "List Type"; ?></label>
                                    <select class="form-control" name="list_type">
                                        <option value=""><?php echo $this->lang->line('select') ?></option>
                                        <option value="All" <?php
                                                                if ((isset($list_type)) && ($list_type == "All")) {
                                                                    echo "selected";
                                                                }
                                                                ?>><?php echo "All"; ?></option>                                    </select>
                                    <span class="text-danger"><?php echo form_error('list_type'); ?></span>
                                </div>
                            </div>                                                    
                            <div id='date_result'>
                            </div>
                    ...
                    <?php
                        ...
                        <form action="<?php echo base_url('admin/feemaster/updateStatus') ?>" method="post" id="updatereconcile">
                            <div class="">
                                <div class="row">
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label><?php echo "Reconciled On"; ?><small class="req"> *</small></label>
                                            <select class="form-control" name="reconciled_on">
                                                <option value=""><?php echo $this->lang->line('select') ?></option>
                                                <?php if (!empty($banklists)) {
                                                    foreach ($banklists as  $bankRow) {
                                                ?>
                                                        <option value="<?php echo $bankRow['id']; ?>"><?php echo $bankRow['bank_head']; ?></option>
                                                <?php  }
                                                } ?>
                                            </select>
                                            <span class="text-danger"><?php echo form_error('reconciled_on'); ?></span>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label><?php echo "Date"; ?></label>
                                            <input type="text" name="pass_date" class="form-control date_fee" id="pass_date" value="<?php echo date('d-m-Y'); ?>">
                                            <span class="text-danger"><?php echo form_error('pass_date'); ?></span>
                                        </div>
                                    </div>
                                </div>
                                <div class="box-header ptbnull"></div>
                                <div class="box-header ptbnull">
                                    <h3 class="box-title titlefix"><i class="fa fa-money"></i> <?php ?> <?php echo $this->lang->line('fees') . " " . $this->lang->line('collection') . " "; ?> From <?php echo date('d/m/Y', strtotime($start_date)) . " To " . date('d/m/y', strtotime($end_date)); ?></h3>
                                    <div class="pull-right">
                                        <button type="submit" name="submit" value="submit" class="btn btn-primary btn-sm pull-right checkbox-toggle" autocomplete="off"><i class="fa fa-save"></i> Save </button>
                                    </div>
                                </div>
                                    ...
                                    <table class="table table-striped table-bordered table-hover " id="headerTable">
                                        <thead class="header">
                                            <tr>
                                                ...
                                                <th><?php echo "Pass " . $this->lang->line('date'); ?></th>
                                        ...
                                        <tbody>
                                            ...
                                            ?>
                                                <tr style="<?php echo $color; ?>">
                                                    <td class="text-left-md">
                                                        <input type="checkbox" name="receipt_id[]" id="receipt_id" value="<?php echo $value->fee_receipt_id; ?>">
                                                    </td>
                                                    <td style="mso-number-format:'\@'" class="text-left-md payment_id"><?php echo $value->fee_receipt_id; ?></td>

                                                    <td class="text-left-md"><?php echo date('d-m-Y', strtotime($value->receipt_date)); ?></td>
                                                    <td class="text-left-md"><?php echo $value->pass_date != "" ? date('d-m-Y', strtotime($value->pass_date)) : ""; ?></td>
                                                    ...
                                                    <td class="text-left-md"><?php echo $value->payment_mode;
                                                                                echo "<br>Note: " . $value->note;

                                                                                ?></td>

                                                    <td class="text-left"><?php echo $value->fee_receipt_status;
                                                                            if ($value->fee_receipt_status == "Cancelled") {
                                                                                echo "<br>" . $value->cancel_reason;
                                                                            } ?></td>
                                                    ...
                                                    <!-- <td class="text-right"><?php //echo $value->gross_amount; 
                                                                                ?></td> -->
                                                    <!-- <td class="text-right"><?php //echo $value->discount; 
                                                                                ?></td> -->
                                                    <!-- <td class="text-right"><?php //echo $value->fine; 
                                                                                ?></td> -->
                                                ...
                                                <td class="text-right">
                                                    ...
                                                    <button class="btn btn-default btn-xs " id="revert" data-invoiceno="<?php echo $value->fee_receipt_id; ?>" data-main_invoice="<?php echo $value->fee_receipt_id; ?>" data-sub_invoice="<?php echo $value->fee_receipt_id; ?>" data-session_id="<?php echo $value->session_id; ?>" data-toggle="modal" data-target="#confirm-delete" title="<?php echo $this->lang->line('revert'); ?>">
                                                        <i class="fa fa-undo"> </i>
                                                    </button>
                                                    
                                                    <button class="btn btn-xs btn-default" id="editbtn" data-id="<?php echo  $value->fee_receipt_id;   ?>" data-mode="<?php echo $value->payment_mode; ?>" data-note="<?php echo $value->note; ?>" data-toggle="modal" data-target="#edit_receipt"  title="<?php echo "Edit"; ?>">
                                                        <i class="fa fa-edit"></i>
                                                    </button>
                                                <?php }  ?>
                                                <button class="btn btn-xs btn-default printDoc" id="printbtn" data-id="<?php echo  $value->fee_receipt_id;   ?>" title="<?php echo $this->lang->line('print'); ?>">
                                            ...
                                            <tr>
                                                ...
                                                <!-- <td class="text text-right " style="font-weight:bold"><?php //echo $grdTotalTotal; 
                                                                                                            ?></td>
                                                <td class="text text-right" style="font-weight:bold"><?php //echo $grddiscountTotal; 
                                                                                                        ?></td>
                                                <td class="text text-right " style="font-weight:bold"><?php //echo $grdfineTotal; 
                                                                                                        ?></td> -->
                                                <td></td>
...
<script type="text/javascript">
    $(document).ready(function() {
        $("input[name='checkAll']").click(function() {
            $("input[name='receipt_id[]']").not(this).prop('checked', this.checked);
        });
...
<script>
    ...
    $(document).ready(function() {
        ...
        $("#updatereconcile").submit(function(e) {
            e.preventDefault(); // avoid to execute the actual submit of the form.
            var checkCount = $("input[name='receipt_id[]']:checked").length;

            if (checkCount == 0) {
                alert("<?php echo "Atleast Select One"; ?>");
