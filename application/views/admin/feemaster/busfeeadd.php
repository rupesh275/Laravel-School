                                    ...
                                    foreach ($student_due_fee as $key => $val) {
                                        ...
                                        foreach ($val as $key => $fee) {
                                            ...
                                            foreach ($fee->fees as $fee_key => $fee_value) {
                                                //         echo "<pre>";
                                                // print_r($fee);
                                                if ($userdata['user_type'] == "Super Admin" || $userdata['user_type'] == "Admin" || $userdata['user_type'] == "Accountant" || $userdata['user_type'] == "OfficeAdmin" || $userdata['user_type'] == "Bus Operator") {
                                    ...
                                    ?>
                                                            ...
                                                            if ($old_group != $fee_value->name) {
                                                                ?>
                                                                    ...
                                                                    <td><?php echo $fee_value->name;
                                                                        $old_group = $fee_value->name; ?>
                                                                    </td>
...
<script type="text/javascript">
    $(document).on('click', '.save_button', function(e) {
        ...
        var discount_amt = $(this).data('discount_amt');
        $.ajax({
            url: '<?php echo site_url("admin/busfee/addbusstudentfee") ?>',
            type: 'post',
            data: {
                action: action,
                student_session_id: student_session_id,
...
<script type="text/javascript">
    $("#myFeesModal").on('shown.bs.modal', function(e) {
        ...
        $.ajax({
            type: "post",
            url: '<?php echo site_url("admin/busfee/geBalanceFee") ?>',
            dataType: 'JSON',
            data: {
                'fee_groups_feetype_id': fee_groups_feetype_id,
                'student_fees_master_id': student_fees_master_id,
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on('click', '.collectSelected', function() {
            ...
            $.ajax({
                type: 'POST',
                url: base_url + "admin/busfee/getcollectfeemodel",
                data: {
        ...
        $(document).on('change', "#discount_amt", function() {
            ...
            if (fees_type == "m") {
                if (amount > 0) {
                    ...
                    if (percent > 0) {
                        disc_amt = Math.round(parseFloat(total_main_fees) * (percent / 100), 2);
                    }
                }
    ...
    function collect_fee_group(print) {
        var form = $("#collect_fee_group");
        // var url = form.attr('action');
        var url = '<?php echo site_url('admin/busfee/addbusfeegrp'); ?>';
        var smt_btn = $(this).find("button[type=submit]");
        var total_paid = $('input#total_paid').val();
