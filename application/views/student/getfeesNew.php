                            ...
                            <div class="tab-pane <?php
                                ...
                                ?>
                                        <div class="studentname">
                                            <a class="" href="<?php echo base_url() . "user/user/addfee/" . $stvalue["id"] ?>">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-8">
                                <div class="btn-group pull-right">
                                    <a href="<?php echo base_url() ?>user/user/getfees" type="button" class="btn btn-primary btn-xs">
                    ...
                    <div class="box-body" style="padding-top:0;">
                        ...
                        <div class="row no-print">
                            <div class="col-md-12 mDMb10">
                                ...
                                <button type="button" class="btn btn-sm btn-warning collectSelected" data-student_session_id="<?php echo $student['student_session_id']; ?>" id="load" data-loading-text="<i class='fa fa-spinner fa-spin '></i> <?php echo $this->lang->line('please_wait') ?>"><i class="fa fa-money"></i> <?php echo $this->lang->line('pay') ?></button>
                                                                ...
                                                                ?>
                                                                        <tr class="white-td">
                                                                            ...
                                                                            <td class="text text-right">
                                                                                <div class="btn-group ">
                                                                                    <div class="pull-right">
                                                                                        <button class="btn btn-xs btn-default printDoc1" data-student_session_id="<?php echo $student_session_id[$i]; ?>" data-percentage="<?php //echo $fee_value->
                                                                                    ...
                                                                                    <div class="btn-group ">
                                                                                        <div class="pull-right">
                                                                                            ...
                                                                                            <button class="btn btn-xs btn-default printDocDirect" data-student_session_id="<?php echo $student_session_id[$i]; ?>" data-percentage="<?php //echo $fee_value->
                                                                                                                                                                                                                                ?>" data-discount_amt="<?php echo $fee_deposits_value->amount_discount; ?>" data-payment_id="<?php echo  $fee_deposits_value->inv_no; ?>" data-full_amount="<?php echo $total_amount; ?>" data-fee_master_id="<?php echo $fee_value->id ?>" data-fee_groups_feetype_id="<?php echo $fee_value->fee_groups_feetype_id ?>" data-paidamt="<?php echo $fee_paid; ?>" data-main_invoice="<?php echo $fee_value->student_fees_deposite_id ?>" data-sub_invoice="<?php echo $fee_deposits_value->inv_no ?>"   data-id="<?php echo $fee_deposits_value->inv_no ?>"   data-sessionid="<?php if(isset($fee_deposits_value->session_id)) { echo  $fee_deposits_value->session_id; } else { echo "0"; }   ?>" title="<?php echo $this->lang->line('print'); ?>"><i class="fa fa-print"></i> </button>
                                                                                        </div>
                                                                                    </div>
                                                                                </td>
                                                                            </tr>
...
<div id="listCollectionModal" class="modal fade">
    <div id="body" class="modal-dialog">
        ...
        <form id="collect_fee_group">
            <!-- <form action="<?php echo site_url('user/user/addfeegrp'); ?>" method="POST" id="collect_fee_group"> -->
            <div class="modal-content">
                <!-- //================ -->
...
<script type="text/javascript">
    $(document).ready(function() {
        $(document).on('click', '.printDocDirect', function() {
            ...
            $.ajax({
                url: '<?php echo site_url("user/user/printFeesByName_previous_id_get") ?>',
                type: 'post',
                data: {
                    'id': id,
        ...
        $(document).on('click', '.printDoc', function() {
            ...
            $.ajax({
                url: '<?php echo site_url("user/user/printFeesByName") ?>',
                type: 'post',
                data: {
        ...
        $(document).on('click', '.printInv', function() {
            ...
            var fee_groups_feetype_id = $(this).data('fee_groups_feetype_id');
            $.ajax({
                url: '<?php echo site_url("user/user/printFeesByGroup") ?>',
                type: 'post',
                data: {
                    'fee_groups_feetype_id': fee_groups_feetype_id,
                    'fee_master_id': fee_master_id,
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#confirm-delete').on('click', '.btn-ok', function(e) {
            ...
            $.ajax({
                type: "post",
                url: '<?php echo site_url("user/user/deleteFee") ?>',
                dataType: 'JSON',
                data: {
                    'main_invoice': main_invoice,
                    'sub_invoice': sub_invoice
        ...
        $('#confirm-discountdelete').on('click', '.btn-discountdel', function(e) {
            ...
            $.ajax({
                type: "post",
                url: '<?php echo site_url("user/user/deleteStudentDiscount") ?>',
                dataType: 'JSON',
                data: {
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on('click', '.collectSelected', function() {
            ...
            if(total_main_fees==fee_balance_total)
            {
                enable_auto_disc = 0;
            }
            $.ajax({
                ...
                $('.item_count').each(function(index,element) {
                    ...
                    if(balance_amount>0)
                    {
                        ...
                        else
                        {
                            ...
                            $('#dis_fee_amount_' + itm_cnt).html("Rs." + balance_amount);
                            balance_amount=0;
                        }
                    }
                    else
    ...
    {
        var total_balance_fees =  $('input#total_balance').val();
        var paid_amt  =  $('input#paid_amount').val();
        var lafdenabled = parseFloat($("#lafd_disc").data('lafdenabled'));
        var sibenabled = parseFloat($("#sib_disc").data('sibenabled'));
        if(parseFloat(total_balance_fees) == parseFloat(paid_amt))
        {
            if(lafdenabled == 1)
            {
                apply_lafd();
            }
            else if(sibenabled == 1)
            {
                apply_sibling();
            }
            else
    ...
    function apply_sibling()
    {
        var otenabled = parseFloat($("#sib_disc").data('lafdenabled'));
            var amount = parseFloat($("#sib_disc").data('dis_amt'));
            var discid = parseFloat($('#sib_disc').data('discid'));
            
            var fee_amount = parseFloat($('.total_amt_').val());
            var percent = $('#sib_disc').data('per_cent');
            var total_main_fees = $('#sib_disc').data('total_main_fees');
            var total_other_fees = $('#sib_disc').data('total_other_fees');
            var fees_type = $('#sib_disc').data('fees_type');
            var code = $('#sib_disc').data('code');
            var am=0;
            var balance_amount = 0;
            disc_amt=0;
            disc_collected = 0;
    ...
    function apply_lafd()
    {
        var otenabled = parseFloat($("#lafd_disc").data('lafdenabled'));
            var amount = parseFloat($("#lafd_disc").data('dis_amt'));
            var discid = parseFloat($('#lafd_disc', this).data('discid'));
            var fee_amount = parseFloat($('.total_amt_').val());
            var percent = $('#lafd_disc').data('per_cent');
            var total_main_fees = $('#lafd_disc').data('total_main_fees');
            var total_other_fees = $('#lafd_disc').data('total_other_fees');
            var fees_type = $('#lafd_disc').data('fees_type');
            var code = $('#lafd_disc').data('code');
            var am=0;
            var balance_amount = 0;
            disc_amt=0;
            disc_collected = 0;
    ...
    function collect_fee_group(print) {
        var form = $("#collect_fee_group");
        // var url = form.attr('action');
        var url = '<?php echo site_url('user/user/addfeegrp'); ?>';
        var smt_btn = $(this).find("button[type=submit]");
        var total_paid = $('input#total_paid').val();
        ...
        $.ajax({
            ...
            url: url,
            dataType: 'JSON',
            data: form.serialize() + "&print=" + print + "&total_paid=" + total_paid + "&total_balance=" + total_balance  + "&paid_amount=" + paid_amount + "&tot_fine_amount=" + tot_fine_amount, // serializes the form's elements.
            beforeSend: function() {
                smt_btn.button('loading');
                $(btn).button('loading');
            },
    ...
    $(document).on("click", ".deleteassignfees", function() {
        ...
        if (confirm("Are you sure?")) {
            $.ajax({
                type: 'POST',
                url: base_url + "user/user/deleteassignfee",
                data: {
                    student_session_id: student_session_id,
                    student_fees_master_id: student_fees_master_id,
                },
