        ...
        <div class="row">
            <div class="col-md-12">
                <div class="box removeboxmius">
                    ...
                    <form role="form" action="<?php echo site_url('report/reconsile_report') ?>" method="post" class="">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="pass_status">
                                        ...
                                        <option value="Active" <?php
                                                                if ((isset($pass_status)) && ($pass_status == "Active")) {
                                                                    echo "selected";
                                                                }
                                                                ?>>Active</option>
                                        <option value="Cancelled" <?php
                                                                if ((isset($pass_status)) && ($pass_status == "Cancelled")) {
                                                                    echo "selected";
                                                                }
                                                                ?>>Cancelled</option>
                                        <option value="Bounced" <?php
                                                                if ((isset($pass_status)) && ($pass_status == "Bounced")) {
                                                                    echo "selected";
                                                                }
                                                                ?>>Bounced</option>
                                        <option value="Cleared" <?php
                                                                if ((isset($pass_status)) && ($pass_status == "Cleared")) {
                                                                    echo "selected";
                                                                }
                                                                ?>>Cleared</option>
                                    </select>
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                            <tr style = "<?php echo $color;?>">
                                                ...
                                                <td class="text-right"><?php echo $value->gross_amount; ?><br></td>
                                                <td class="text-right">
                                                    ...
                                                    <button class="btn btn-xs btn-default" id="updatebtn" data-id="<?php echo  $value->fee_receipt_id;   ?>" data-pass_status="<?php echo $value->passStatus; ?>" data-pass_date="<?php if($value->pass_date != null){ echo date('d-m-Y',strtotime($value->pass_date));}else{ echo "";} ?>" data-toggle="modal" data-target="#update_receipt"  title="<?php echo "Update"; ?>">
...
<!-- Model Update -->
<div class="updatemodal modal fade" id="update_receipt" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-group">
                        <label for="inputPassword3" class="col-sm-3 control-label"> <?php echo $this->lang->line('date'); ?></label>
                        <div class="col-sm-9">
                            <input type="text" class="form-control datepass" id="pass_date2" name="pass_date" value=""/> 
                            <span class="text-danger" id="payment_mode_error"></span>
                        </div>
                        
                        <span id="form_collection_pass_date_error" class="text text-danger"></span>
                    </div>
                    <br>
                    <div class="form-group">
                        <label for="inputPassword3" class="col-sm-3 control-label"> <?php echo $this->lang->line('status') ?></label>
                        <div class="col-sm-9">
                            <select name="pass_status" id="pass_status2" class="form-control">
                                <option value="Active">Active</option>
                                <option value="Cancelled">Cancelled</option>
                                <option value="Bounced">Bounced</option>
                                <option value="Cleared">Cleared</option>
                            </select>
                            <span id="form_collection_pass_status_error" class="text text-danger"></span>
                        </div>
                    </div>

            </div>            
            <div class="modal-footer">
                <input type="hidden" name="inv_idss" id="invoice_id2" value="">
                <button type="button" class="btn btn-default" data-dismiss="modal"><?php echo $this->lang->line('cancel'); ?></button>
                <a class="btn btn-danger btn-ok2"><?php echo "Update"; ?></a>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#update_receipt').on('show.bs.modal', function(e) {
            var invid = $(e.relatedTarget).data('id');
            var passStatus = $(e.relatedTarget).data('pass_status');
            var pass_date = $(e.relatedTarget).data('pass_date');
            $('#invoice_id2').val(invid);
            $('#pass_date2').val(pass_date);
            $('#pass_status2').val(passStatus);
        ...
        $('#update_receipt').on('click', '.btn-ok2', function(e) {
            var $modalDiv = $(e.delegateTarget);
            var sub_invoice = $('#invoice_id2').val();
            var pass_status = $("#pass_status2 option:selected").text();
            var pass_date = $('#pass_date2').val();
            $modalDiv.addClass('modalloading');
            $.ajax({
                type: "post",
                url: '<?php echo site_url("student/updateReceiptstatus") ?>',
                dataType: 'JSON',
                data: {
                    'sub_invoice': sub_invoice,
