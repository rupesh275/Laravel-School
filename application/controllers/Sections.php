...
class Sections extends Admin_Controller
{
    ...
    public function getClassTeacherSectionArray()
    {
        $class_id = $this->input->get('class_id');
        $data     = array();
        $userdata = $this->customlib->getUserData();
        if (($userdata["role_id"] == 2)) {
            $id    = $userdata["id"];
            $data = $this->db->join('sections','sections.id = class_teacher.section_id')->where("staff_id", $id)->where("class_id", $class_id)->get("class_teacher")->result_array();
