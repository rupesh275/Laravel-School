...
class Facility extends Admin_Controller
{
    ...
    {
        if (!$this->rbac->hasPrivilege('facility', 'can_view')) {
            access_denied();
        }
        $data = array();
        ...
        $this->load->view('admin/front/facility/index', $data);
        $this->load->view('layout/footer');
    }

    public function create()
    {
        if (!$this->rbac->hasPrivilege('facility', 'can_add')) {
            access_denied();
        }
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view('layout/header');
            $this->load->view('admin/front/facility/create', $data);
            $this->load->view('layout/footer');
        } else {
            ...
            $data = array(
                ...
                'type'             => $this->config->item('ci_front_facility_content'),
            ...
            redirect('admin/front/facility');
        }
    }

    public function edit($slug)
    {
        if (!$this->rbac->hasPrivilege('facility', 'can_edit')) {
            access_denied();
        }
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('layout/header');
            $this->load->view('admin/front/facility/edit', $data);
            $this->load->view('layout/footer');
        } else {
    ...
    public function delete($id)
    {
        ...
        if (!$this->rbac->hasPrivilege('facility', 'can_delete')) {
            access_denied();
        }
        $data['title'] = 'Facility';
        $this->cms_program_model->removeBySlug(urldecode($id), 'facility');
        redirect('admin/front/facility');
    }
