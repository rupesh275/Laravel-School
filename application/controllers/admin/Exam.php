...
class Exam extends Admin_Controller {
    ...
    public function update_attaendance()
    {
        // ini_set('display_errors', 1);
        // ini_set('display_startup_errors', 1);
        // error_reporting(E_ALL);
        $result = $this->db->query('select student_session.student_id,student_session.id,students.admission_date from student_session inner join students on students.id = student_session.student_id where student_session.class_id >=4 and student_session.class_id <= 8');
        if(empty($result))
        {
            echo "empty";die();
        }
        ...
        foreach ($result as $key => $value) {
            //echo "select * from student_attendance where student_session_id = '" . $value['id'] . "' and date <= '".$value['admission_date']. "'";
            $atten = $this->db->query("select * from student_attendences where student_session_id = '" . $value['id'] . "' and date < '".$value['admission_date']. "' order by date desc");
            if(empty($atten))
            {
            ...
            {
                $this->db->query("delete from student_attendences where student_session_id = '" . $value['id'] . "' and date < '".$value['admission_date']. "'");
                echo "<br>" . $value['id'];
            }
