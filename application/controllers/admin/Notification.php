...
class Notification extends Admin_Controller
{
    ...
    public function savetemplate()
    {
        ...
        $this->form_validation->set_rules('template_message', $this->lang->line('template_message'), 'required|trim');
    ...
    {
        $roles_ids = '1,8';
        $notification = $this->notification_model->getNotificationList($roles_ids);
        ...
        // $daysArray = explode(',',$reminder['reminder_period']);
        // $todayDate = "15-06-2023";//date('d-m-Y');

        // if (!empty($reminderDate)) {
        //    foreach ($daysArray as  $value) {
        //     $reminderdate = date('d-m-Y',strtotime('-'.$value.' days'));
    ...
    public function all($status = 0)
    {
        ...
        $this->session->set_userdata('top_menu', 'Communicate');
        $this->session->set_userdata('sub_menu', 'notification/all');
        $data['title'] = 'All Notifications';

        $data['notificationlist'] = $this->notification_model->getNotification($status);
        $data['status'] = $status;
        $this->load->view('layout/header', $data);
        $this->load->view('admin/notification/allnotification', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    {
        $id = $this->input->post('id');
        $this->notification_model->update_notification($id);
