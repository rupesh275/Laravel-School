...
class Teacher extends Admin_Controller {
    ...
    public function delete($id) {
        if (!$this->rbac->hasPrivilege('assign_sub_teacher', 'can_delete')) {
            access_denied();
        }
        $data['title'] = 'Teacher List';
    ...
    public function create() {
        if (!$this->rbac->hasPrivilege('assign_sub_teacher', 'can_add')) {
            access_denied();
        }
        $data['title'] = 'Add teacher';
        $genderList = $this->customlib->getGender();
    ...
    public function edit($id) {
        ...
        if (!$this->rbac->hasPrivilege('assign_sub_teacher', 'can_edit')) {
            access_denied();
        }
    ...
    public function working_days_entry()
    {
        if (!$this->rbac->hasPrivilege('working_days_entry', 'can_edit')) {
            access_denied();
        }

        $this->session->set_userdata('top_menu', 'Academics');
        $this->session->set_userdata('sub_menu', 'Academics/worling_days_entry');
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view('layout/header', $data);
            $this->load->view('working_entry/working_days_entry', $data);
            $this->load->view('layout/footer', $data);
        } else {
            ...
            if (empty($id)) {
                $this->teacher_model->addworkentry($dataArr);
            }else {
                // echo "<pre>";
                // print_r($dataArr);
                // echo "else";die;
                $this->teacher_model->updateworkentry($dataArr);
            }
            $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">' . $this->lang->line('success_message') . '</div>');
            redirect('admin/teacher/working_days_entry');
        }
