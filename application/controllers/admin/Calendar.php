...
class Calendar extends Admin_Controller
{
        ...
        if (!empty($result)) {
            ...
            foreach ($result as $key => $value) {
                ...
                if ($event_type == 'task') {
                    ...
                    if (($event_for == $value["event_for"]) && ($value["role_id"] == $userdata["role_id"]) && ($value['check'] == 1)) {
    ...
    public function usernotes()
    {
        ...
        $userdata = $this->customlib->getUserData();
        $data['result']   = $this->calendar_model->getNotes($userdata['id']);
        $this->form_validation->set_rules('note', "Note", 'trim|xss_clean');
        if ($this->form_validation->run() == FALSE) {
            $this->load->view("layout/header.php");
            $this->load->view("setting/usernotes", $data);
            $this->load->view("layout/footer.php");
        } else {
            ...
            $this->calendar_model->saveNotes($arrayData);
            
            $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">' . $this->lang->line('success_message') . '</div>');
            redirect('admin/calendar/usernotes');
