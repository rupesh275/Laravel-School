...
class Welcome extends Front_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(array('cms_page_model','frontcms_setting_model', 'complaint_Model', 'Visitors_model', 'onlinestudent_model', 'filetype_model', 'customfield_model','enquiry_model','staff_model'));
    ...
    public function index()
    {
        ...
        $this->data['gurudev']           = $this->cms_page_model->getBySlug('IndexGuru');
        $this->data['aboutus']           = $this->cms_page_model->getBySlug('aboutusindex');
        $this->data['princmsg']           = $this->cms_page_model->getBySlug('principalmessageindex');
        
        $this->data['index_preprimary']           = $this->cms_page_model->getBySlug('index-pre-primary');
        $this->data['index_primary']           = $this->cms_page_model->getBySlug('index-primary');
        $this->data['index_secondary']           = $this->cms_page_model->getBySlug('index-secondary');
        
        $this->data['facility']     = $this->cms_program_model->getByCategory('facility');
        $this->data['celebrations']     = $this->cms_program_model->getByCategory('co-curriculam');
        $this->data['testimonials']     = $this->cms_program_model->getByCategory('testimonials');
        $this->data['staff']         = $this->staff_model->listForIndex();
        $this->data['ach']     = $this->cms_program_model->getByCategory('achievements');
        $this->data['result'] = $this->sch_setting_detail; 
        if (!empty($result)) {
            ...
            if (!empty($form)) {
                ...
                if ($this->form_validation->run() == false) {
                    ...
                    if (!empty($setting) && $setting->contact_us_email != "") {
                        ...
                        //by manoj
                        $this->mailer->send_mail_normal($record['email'], $email_subject, $mail_body);
                    }
    ...
    { 
            ...
            $this->data['active_menu'] = 'faculty';
            $page                      = array('title' => 'Faculty', 'meta_title' => 'Faculty', 'meta_keyword' => 'Faculty', 'meta_description' => 'Faculty');
            ...
            $this->data['title']          = 'Faculty';
            $this->data['title_list']     = 'Faculty';
            $this->data['sch_setting']    = $this->sch_setting_detail;
            $this->data['faculty_princ']  =  $this->staff_model->getPrincipal();
            $this->data['faculty_secondary']  =  $this->staff_model->getDepartmentStaffAll(3);
            $this->data['faculty_primary']  =  $this->staff_model->getDepartmentStaffAll(2);
            $this->data['faculty_preprimary']  =  $this->staff_model->getDepartmentStaffAll(1);
            $this->data['faculty_admin'] =  $this->staff_model->getDepartmentStaffAll(4);
            $sch_setting                  = $this->sch_setting_detail;
            $this->load_theme('pages/faculty', $this->config->item('front_layout'));
    }
    public function curriculam($ids)
    { 

            $this->data['active_menu'] = 'caricculam';
            $page                      = array('title' => 'Curriculam', 'meta_title' => 'Curriculam', 'meta_keyword' => 'Curriculam', 'meta_description' => 'Curriculam');
            ...
            $this->data['sec_pre_primary']  =  $this->cms_page_model->get(27);
            $this->data['sec_primary']  =  $this->cms_page_model->get(28);
            $this->data['sec_secondary']  =  $this->cms_page_model->get(30);
            $this->data['ids']  =  $ids;
            $sch_setting                  = $this->sch_setting_detail;
            $this->load_theme('pages/curriculam', $this->config->item('front_layout'));
    }    
        ...
        if ($this->module_lib->hasActive('online_admission')) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                if ($document_validate) {
                    ...
                    $this->data['gender']              = $this->input->post('guardian_name');
    ...
    public function checktermcondition()
   {
    ...
    public function view_payroll($hash_id)
    {

        $payrollRow = $this->payroll_model->getPayslipByHash($hash_id);
        $data['sch_setting'] = $this->sch_setting_detail;
        $data['hash_id']     = $hash_id;
        $data['staff_id']= $staff_id = $payrollRow['staff_id'];
        $data['month']= $month = $payrollRow['month'];
        $data['year']= $year = $payrollRow['year'];
        ...
        $this->load->view('admin/payroll/payslipNewMail', $data);
    }
