...
class Subjecttimetable_model extends MY_Model
{
    ...
    public function canAddExamMarksTeacher($staff_id, $class_id, $section_id, $subject_id, $mainsub_id)
    {
        $class_teacher = $this->db->select('*')->from('teacher_subjects')->join('class_sections','class_sections.id = teacher_subjects.class_section_id')->where('teacher_id', $staff_id)->where('subject_id', $mainsub_id)->where('class_sections.class_id',$class_id)->where('class_sections.section_id',$section_id)->get()->num_rows();
        if ($class_teacher > 0) {
            return 1;
