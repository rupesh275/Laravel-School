...
class Cms_program_model extends MY_Model
{
    ...
    public function getByCommon($category = null, $params = array())
    {
        $this->db->select('*');
        $this->db->from('front_cms_programs');
        $this->db->order_by('created_at', 'desc');
        $arraydata = array("gallery","standard","banner","notice","events");
            $this->db->where_not_in('type', $arraydata);
        if (array_key_exists("start", $params) && array_key_exists("limit", $params)) {
            $this->db->limit($params['limit'], $params['start']);
        } elseif (!array_key_exists("start", $params) && array_key_exists("limit", $params)) {
            $this->db->limit($params['limit']);
    ...
    public function removeByid($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete('front_cms_programs');
        $message   = DELETE_RECORD_CONSTANT . " On event title " . $id;
        $action    = "Delete";
        $record_id = $id;
        $this->log($message, $record_id, $action);
        //======================Code End==============================
