...
class Emailconfig_model extends MY_Model {
    ...
    public function addWati($data) {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $this->db->select()->from('wati_config');
        $q = $this->db->get();
        if ($q->num_rows() > 0) {
            $result = $q->row();
            ...
            $this->db->where('id', $result->id);
            $this->db->update('wati_config', $data);
            $message = UPDATE_RECORD_CONSTANT . " On  wati config id " . $result->id;
            $action = "Update";
            $record_id = $result->id;
            $this->log($message, $record_id, $action);
            //======================Code End==============================
            ...
            $this->db->insert('wati_config', $data);
            $insert_id = $this->db->insert_id();
            $message = INSERT_RECORD_CONSTANT . " On wati config id " . $insert_id;
            $action = "Insert";
            $record_id = $insert_id;
            $this->log($message, $record_id, $action);          
            //======================Code End==============================
