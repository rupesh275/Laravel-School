...
class Batchsubject_model extends CI_Model {
    ...
    //        return $result;
    //    }
    public function getExamMainSubjects($id = null,$main_sub=null) {
        
        $res=$this->db->query("select id,name,code,type from subjects where id in (select distinct main_sub from exam_group_class_batch_exam_subjects where exam_group_class_batch_exams_id = '".$id."') order by orderIndex")->result();
        $rw=0;
        foreach($res as $r)
        {
            //echo "<br>select * from subjects where id in (select id from exam_group_class_batch_exam_subjects where  exam_group_class_batch_exams_id = '".$id."' and main_sub = '".$r->id."' )";
            $res[$rw]->subGroups = $this->db->query("select  id,name,code,type  from subjects where id in (select distinct parent_id from subjects where id in (select subject_id from exam_group_class_batch_exam_subjects where  exam_group_class_batch_exams_id = '".$id."' and main_sub = '".$r->id."' ))")->result();
            $rww=0;
            foreach($res[$rw]->subGroups as $subgroup)
            {
                $res[$rw]->subGroups[$rww]->subSubjects = $this->db->query("select  id,name,code,type  from subjects where parent_id = '$subgroup->id' and  id in (select subject_id from exam_group_class_batch_exam_subjects where  exam_group_class_batch_exams_id = '".$id."' and main_sub = '".$r->id."' )")->result();
                ++$rww;
            }
            ++$rw;
        }
    ...
    public function getExamMainSubjectsByid($id = null,$main_sub=null) {
        
        $res=$this->db->query("select id,name,code,type from subjects where id in (select distinct main_sub from exam_group_class_batch_exam_subjects where exam_group_class_batch_exams_id = '".$id."') order by id")->result();
        $rw=0;
        foreach($res as $r)
        {
            //echo "<br>select * from subjects where id in (select id from exam_group_class_batch_exam_subjects where  exam_group_class_batch_exams_id = '".$id."' and main_sub = '".$r->id."' )";
            $res[$rw]->subGroups = $this->db->query("select  id,name,code,type  from subjects where id in (select distinct parent_id from subjects where id in (select subject_id from exam_group_class_batch_exam_subjects where  exam_group_class_batch_exams_id = '".$id."' and main_sub = '".$r->id."' order by id))")->result();
            $rww=0;
            foreach($res[$rw]->subGroups as $subgroup)
            {
                $res[$rw]->subGroups[$rww]->subSubjects = $this->db->query("select  id,name,code,type  from subjects where parent_id = '$subgroup->id' and  id in (select subject_id from exam_group_class_batch_exam_subjects where  exam_group_class_batch_exams_id = '".$id."' and main_sub = '".$r->id."' order by id)")->result();
                ++$rww;
            }
            ++$rw;
        }
    ...
    public function getExamMainSubjectsOnly($exam_id)
    {
        $res=$this->db->query("select id,name,code,type from subjects where  id in (select distinct main_sub from exam_group_class_batch_exam_subjects where exam_group_class_batch_exams_id = '".$exam_id."') order by orderIndex")->result();
        return $res;
    }
    public function getExamMainGradeSubjectsOnly($exam_id)
    {
        $res=$this->db->query("select id,name,code,type from subjects where SubjectType = 'Grade' and  id in (select distinct main_sub from exam_group_class_batch_exam_subjects where exam_group_class_batch_exams_id = '".$exam_id."') order by orderIndex")->result();
        return $res;
    }
    public function getExamMainAIPSubjects($exam_id)
    {
        $res=$this->db->query("select id,name,code,type from subjects where short_name like '%AIP%' and  id in (select subject_id from exam_group_class_batch_exam_subjects where exam_group_class_batch_exams_id = '".$exam_id."') order by orderIndex")->result();
        return $res;
    }    
    public function getExamSubjects($id = null,$main_sub=null) {
        ...
        $this->db->where('teacher_id',$session_id);
        $query=$this->db->get('teacher_subjects')->row_array();
        ...
        if ($main_sub != null) {
            $this->db->where('exam_group_class_batch_exam_subjects.main_sub', $main_sub);
        }
        // if ($role_id == 2) {
        //     $this->db->where('exam_group_class_batch_exam_subjects.main_sub', $query['subject_id']);
        // }

        $this->db->order_by('exam_group_class_batch_exam_subjects.main_sub,exam_group_class_batch_exam_subjects.id');
    ...
    public function getExamSubjectsOnly($id,$subjectid,$main_sub=null) {
        ...
        $this->db->join('subjects', 'subjects.id = exam_group_class_batch_exam_subjects.subject_id');
        $this->db->where('exam_group_class_batch_exam_subjects.exam_group_class_batch_exams_id', $id);
        $this->db->where('exam_group_class_batch_exam_subjects.subject_id', $subjectid);
        if ($subject_condition == 1) {
            $this->db->where_in('subjects.id', $my_subjects);
        }
        if ($main_sub != null) {
    ...
    public function getExamstudentSubjectsBYMainSubject($exam_id,$main_sub_id) {
        $this->db->select('exam_group_class_batch_exam_subjects.*,subjects.name as `subject_name`,subjects.code as `subject_code`,subjects.type as `subject_type`')->from('exam_group_class_batch_exam_subjects');

        $this->db->join('subjects', 'subjects.id = exam_group_class_batch_exam_subjects.subject_id');
        $this->db->where('exam_group_class_batch_exam_subjects.exam_group_class_batch_exams_id', $exam_id);
        $this->db->where('exam_group_class_batch_exam_subjects.main_sub', $main_sub_id);
    ...
    public function getExamSubjectsBySubjectId($exam_id,$exam_subject_id) {

        $sql = "SELECT exam_group_class_batch_exam_subjects.*,subjects.name as `subject_name`,subjects.code FROM `exam_group_class_batch_exam_subjects` INNER JOIN subjects on subjects.id=exam_group_class_batch_exam_subjects.subject_id WHERE exam_group_class_batch_exam_subjects.exam_group_class_batch_exams_id = '$exam_id' and exam_group_class_batch_exam_subjects.subject_id=" . $this->db->escape_str($exam_subject_id);
        $query = $this->db->query($sql);
        return $query->row();
    }
