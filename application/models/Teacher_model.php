...
class Teacher_model extends CI_Model {
    ...
    public function getMemberTeacher() {
        $this->db->select('staff.*, IFNULL(libarary_members.id,0) as `libarary_member_id`, IFNULL(libarary_members.library_card_no,0) as `library_card_no`')->from('staff');

        $this->db->join('libarary_members', 'libarary_members.member_id = staff.id and libarary_members.member_type = "teacher"');
        $this->db->where('staff.is_active', 1);
        $this->db->order_by('staff.id');
    ...
    public function addworkentry($data)
    {
       $this->db->insert_batch('working_days_entry', $data);
       
    }
    public function updateworkentry($data)
    {
       $this->db->update_batch('working_days_entry', $data, 'id');
    }

    public function get_workingDays($class_id)
    {
        $this->db->where('class_id', $class_id);
        $this->db->where('session_id', $this->current_session);
        return $this->db->get('working_days_entry')->row_array();
        
    }

    public function getTecherClass($staff_id)
    {
        $this->db->where('staff_id', $staff_id);
        $this->db->where('session_id', $this->current_session);
       return $this->db->get('class_teacher')->row_array();
