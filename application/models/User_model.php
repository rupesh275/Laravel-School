...
class User_model extends MY_Model {
    ...
    public function checkParentLogin($data) {
        $userid = $data['username'];
        $password = $data['password'];
        $student = $this->db->query("select parent_id from students where mobileno = '$userid' and password = '$password'")->result_array();
        if (!empty($student)) {
            if(!empty($student))
            {
                $this->db->select('id, username, password,role,is_active,lang_id');
                $this->db->from('users');
                $this->db->where('id', $student[0]['parent_id']);
                $this->db->limit(1);
                $query = $this->db->get();
                if ($query->num_rows() == 1) {
                    return $query->result();
    ...
    public function checkParentLogin_session_id($student_session_id) {
        $student = $this->db->query("select parent_id from students where id in (select student_id from student_session  where id = '$student_session_id') ")->result_array();
        if (!empty($student)) {
            if(!empty($student))
            {
                $this->db->select('id, username, password,role,is_active,lang_id');
    ...
    public function checkOldPassNew($data) {
        $this->db->where('parent_id', $data['user_id']);
        $this->db->where('password', $data['current_pass']);
    ...
    public function saveNewPassNew($data) {
