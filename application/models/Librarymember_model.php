...
class Librarymember_model extends MY_Model {
    ...
    public function getstudent($class_id,$section_id)
    {
        ...
        $this->db->select('libarary_members.*,students.admission_no,firstname,middlename,students.lastname,
        father_phone,guardian_phone,libarary_members.id');
        $this->db->join('students','students.id = libarary_members.member_id');
        $this->db->join('student_session','student_session.student_id = students.id');
        $this->db->where("student_session.class_id",$class_id);
        $this->db->where("student_session.section_id",$section_id);
        $this->db->where("libarary_members.member_type",'student');
        // $this->db->where("student_session.status != ",'4');
        $this->db->where("student_session.session_id",$this->current_session);
        $query = $this->db->get('libarary_members')->result_array();
    ...
    //     print_r ($array);
    //     echo "</pre>";
    //     $this->db->insert('libarary_members',$array);
