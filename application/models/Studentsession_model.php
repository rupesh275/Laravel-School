...
class Studentsession_model extends CI_Model
{
    ...
    public function searchStudents($class_id = null, $section_id = null, $key = null)
    {
        $this->db->select('student_session.id,student_session.student_id,classes.class,sections.section,
            students.firstname,students.middlename,students.lastname,students.admission_no,student_session.roll_no,students.dob,students.guardian_name,
    ...
    {
        $this->db->select('students.aadhar_name,students.admission_no,student_session.roll_no,student_session.session_id, student_session.class_id, student_session.section_id,student_session.id,student_session.student_id,classes.class,classes.code,sections.section,
            students.firstname,students.middlename,students.lastname,students.admission_no,student_session.roll_no,students.dob,students.guardian_name,students.father_name,students.mobileno,students.email')->from('student_session');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->join('sections', 'sections.id = student_session.section_id');
        $this->db->join('students', 'students.id = student_session.student_id');
    ...
    {
        $this->db->select('students.admission_no,student_session.roll_no,student_session.session_id, student_session.class_id, student_session.section_id,student_session.id,student_session.student_id,classes.class,sections.section,
            students.firstname,students.middlename,students.lastname,students.admission_no,student_session.roll_no,students.dob,students.guardian_name,
            ')->from('student_session');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->join('sections', 'sections.id = student_session.section_id');
    ...
    public function update_status($data,$student_id)
    {
        if (isset($student_id)) {
            $this->db->where('student_id', $student_id);
            $this->db->where('session_id', $this->current_session);
            ...
            $message   = UPDATE_RECORD_CONSTANT . " On student_session and student id " . $student_id;
            $action    = "Update";
            $record_id = $insert_id = $student_id;
            // $this->log($message, $record_id, $action);
        } 
    }
    public function update_status_tc($data,$student_id,$student_session_id)
    {
        if (isset($student_id)) {
