...
class Feegrouptype_model extends MY_Model {
    ...
    public function add_class_fees($data) {
		$this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $this->db->where('class_id', $data['class_id']);
        // $this->db->where('section_id', $data['section_id']);
        $this->db->where('fees_group_id', $data['fees_group_id']);
        $this->db->where('session_id', $data['session_id']);
        $q =$this->db->get('class_fees_mst')->num_rows();
        ...
        if ( $q > 0) {
            ...
            $this->db->where('fees_group_id', $data['fees_group_id']);
            $this->db->where('session_id', $data['session_id']);
            $this->db->update('class_fees_mst', $data);
			$message = UPDATE_RECORD_CONSTANT . " On  class_fees_mst " . $data['fees_group_id'];
            $action = "Update";
            $record_id = $data['fees_group_id'];
            $this->log($message, $record_id, $action);
           
        } else {
            $this->db->insert('class_fees_mst', $data);
            $id = $this->db->insert_id();
            $message = INSERT_RECORD_CONSTANT . " On class_fees_mst " . $id;
            $action = "Insert";
            $record_id = $id;
            $this->log($message, $record_id, $action);
    ...
    public function get_class_mst($id)
    {
        $this->db->where('fees_group_id', $id);
        $this->db->where('session_id', $this->current_session);
        return $this->db->get('class_fees_mst')->row_array();
        
    }

    public function get_class_mst2($id)
    {
        $this->db->select('*,class_fees_mst.id as classfees_id');
        $this->db->join('classes', 'classes.id = class_fees_mst.class_id');
        //$this->db->join('sections', 'sections.id = class_fees_mst.section_id');
        $this->db->where('fees_group_id', $id);
        $this->db->where('session_id', $this->current_session);
        return $this->db->get('class_fees_mst')->result_array();
        
    }

    public function remove_mst($id)
    {
        $this->db->where('id', $id);
        $this->db->where('session_id', $this->current_session);
        $this->db->delete('class_fees_mst');
        
    }

    public function getclassfess($class_id,$promoted_session="")
    {
        ...
        if ($promoted_session == "") {
            ...
            $this->db->where('session_id', $promoted_session);
        }
        
       return $this->db->get('class_fees_mst');
        
    }

    public function removeclassfeesmst($class_id,$section_id,$feegroup_id)
    {
        $this->db->where('class_id', $class_id);
        // $this->db->where('section_id', $section_id);
        $this->db->where('fees_group_id',$feegroup_id);
        $this->db->where('session_id', $this->current_session);
        $this->db->delete('class_fees_mst');
        
    }

    public function getmstbyclass_section($class_id,$section_id)
    {
        $this->db->join('classes', 'classes.id = class_fees_mst.class_id');
        //$this->db->join('sections', 'sections.id = class_fees_mst.section_id');
        $this->db->join('fee_session_groups', 'fee_session_groups.id = class_fees_mst.fees_group_id');
