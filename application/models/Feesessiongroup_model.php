...
class Feesessiongroup_model extends MY_Model {
    ...
    public function getFeesByGroup($id = null,$promoted_session= "") {
        $this->db->select('fee_session_groups.*,fee_groups.name as `group_name`,fees_type');
        ...
        if ($promoted_session == "") {
            ...
            $this->db->where('fee_session_groups.session_id', $promoted_session);
        }
    ...
    public function valid_check_exists($str) {
        ...
        if ($this->check_data_exists($fee_groups_id, $feetype_id, $id,$amount)) {
    ...
    function check_data_exists($fee_groups_id, $feetype_id, $id,$amount) {
        $this->db->where('fee_groups_id', $fee_groups_id);
        $this->db->where('session_id', $this->current_session);
        $query = $this->db->get('fee_session_groups');
